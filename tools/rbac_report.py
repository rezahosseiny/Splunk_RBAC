...
ENDPOINT_PURPOSE = {
    ...
    "/servicesNS/-/-/configs/conf-authorize":
        "Read every role stanza with its owning app. The only endpoint that "
        "answers the configuration-drift question; the roles endpoint reports an "
        "empty owning app.",
}
    ...
    for folder in SCAN_DIRS:
        for path in glob.glob(os.path.join(ROOT, folder, "**", "*.py"),
            ...
            for raw in re.findall(r"/servicesNS/[^\s\"'{)]*|/services/[^\s\"'{)]*",
                ...
                clean = re.sub(r"/conf-\{?[a-z_]*\}?", "/conf-{file}", clean)
                if len(clean.split("/")) < 3:
                    continue
                found.setdefault(clean, set()).add(
...
def header(catalog, run, totals, fingerprint, generated):
    ...
    lines = [
        ...
        "",
        f"**Report date:** {us_date(run) if run else 'no test run recorded'}  ",
        "**Generated by:** Splunk_RBAC automated test system  ",
...
def rest_section():
    ...
    if undocumented:
        lines += ["", "### Not yet documented", "",
                  "These are called but have no recorded purpose. Each is a gap in",
                  "this section, not in the harness.", "",
                  "| Interface | Used by |", "|---|---|"]
