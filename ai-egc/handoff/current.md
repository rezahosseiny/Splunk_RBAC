...
## Configuration and test report — 2026-08-19

`tools/rbac_report.py` writes `reports/rbac_report.md`: 19 sections, ~1,790
lines, everything read from the catalog, `reports/resolved_inventory.json`, and
the JUnit output of the last run. Nothing in it is hand-maintained.

Wired into `make test`, so a configuration change followed by a test run produces
a report describing the new configuration and the new result. Verified by
changing an index description and confirming the catalog fingerprint moved from
`ffeda7d735ab` to `46171b5dc5b4`, the new text appeared, and both reverted.

Header carries the report date in US format (the test-run date from the JUnit
timestamp, not the generation clock), "Generated by Splunk_RBAC automated test
system", "Developed by Reza Hosseiny, Vice President Technical Services", and the
Tri-State proprietary notice.

Credentials are shown in full — ADR-015, with Reza's condition that they are
masked or omitted if this is ever promoted to production. `make
report-shareable` writes a masked copy; verified zero passwords in it. Both
reports are gitignored.

The REST section discovers endpoints by scanning the source rather than from a
maintained list, so a new interface appears automatically and an undocumented one
is flagged rather than omitted.

Validated: 19 of 19 table-of-contents anchors resolve, zero malformed table rows,
balanced code fences, two Mermaid diagrams, zero AI references.
